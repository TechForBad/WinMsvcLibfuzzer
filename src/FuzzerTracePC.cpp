...
__declspec(align(64)) ATTRIBUTE_INTERFACE
...
extern "C"
{
	...
	// 编译器将在每个边缘插入代码 __sanitizer_cov_trace_pc_guard(&guard_variable)
	// 每个边都有自己的保护变量Guard
	ATTRIBUTE_INTERFACE
		ATTRIBUTE_NO_SANITIZE_ALL
	...
	// 编译器将在每个边上插入代码 __sanitizer_cov_trace_pc()
	ATTRIBUTE_INTERFACE
		ATTRIBUTE_NO_SANITIZE_ALL
	...
	// -fsanitize-coverage=trace-pc-guard
	// 编译器将此回调作为模块构造函数插入到每个dso中
	// “Start”和“Stop”对应于节的开头和结尾，并带有整个二进制文件（可执行文件或DSO）的保护
	// 每个dso至少调用一次回调，并且可以使用相同的参数多次调用
	ATTRIBUTE_INTERFACE
	...
	// 编译器将创建一个检测的pc的表
	// 需要-fsanitize-coverage=inline-8bit-counters或-fsanitize-coverage=trace-pc-guard
	ATTRIBUTE_INTERFACE
	...
	// 支持数据流引导的fuzz，编译器将在比较指令和switch语句周围插入额外的检测
