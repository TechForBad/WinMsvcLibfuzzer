...
namespace fuzzer {
	...
	void Fuzzer::HandleMalloc(size_t Size) {
		...
		PrintFinalStats();
		exit(Options.ErrorExitCode); // Stop right now.
	}
	...
	void Fuzzer::StaticDeathCallback() {
		...
		// 为了避免代码覆盖率信息在程序结束时无法生成，这里必须手动exit(1)结束程序
		exit(1);
	}
	...
	void Fuzzer::InterruptCallback()
	{
		...
		PrintFinalStats();
		exit(0); // Stop right now, don't perform any at-exit actions.
	}
	...
	NO_SANITIZE_MEMORY
		...
		if (Seconds >= (size_t)Options.UnitTimeoutSec) {
			...
			PrintFinalStats();
			exit(Options.TimeoutExitCode); // Stop right now.
		}
	}
	...
	)
	{
		...
		// 执行用户定义回调LLVMFuzzerTestOneInput
		ExecuteCallback(Data, Size);//----------------------------------------------------->

		UniqFeatureSetTmp.clear();
		size_t FoundUniqFeaturesOfII = 0;

		// 获取Corpus之前更新feature数量
		size_t NumUpdatesBefore = Corpus.NumFeatureUpdates();

		// 遍历每一个feature，逐个添加到Corpus中
		TPC.CollectFeatures([&](size_t Feature)
		{
			// 添加feature，如果添加成功则将feature放入UniqFeatureSetTmp
			if (Corpus.AddFeature(Feature, Size, Options.Shrink))
			{
				UniqFeatureSetTmp.push_back(Feature);
			}

			// 如果设置了ReduceInputs参数，则记录一下在这一次变异运行中发现新feature的数量
		...
		// 获取Corpus在添加完feature之后的更新feature数量
		// 相减得到是否包含新的feature
		size_t NumNewFeatures = Corpus.NumFeatureUpdates() - NumUpdatesBefore;

		// 包含新feature，则添加到语料库并写入文件
		if (NumNewFeatures)
		{
	...
	void Fuzzer::ExecuteCallback(const uint8_t* Data, size_t Size)
	{
		...
		// 拷贝到CurrentUnitData CurrentUnitSize
		...
		CurrentUnitSize = Size;
		{
			...
			int Res = CB(DataCopy, Size);//------------------------------------------>LLVMFuzzerTestOneInput
			RunningUserCallback = false;

			// 执行后时间
			UnitStopTime = system_clock::now();

			// LLVMFuzzerTestOneInput返回只有为0才算执行正确
			(void)Res;
			assert(Res == 0);
	...
	void Fuzzer::TryDetectingAMemoryLeak(const uint8_t* Data, size_t Size,
		...
		if (EF->__lsan_do_recoverable_leak_check()) { // Leak is found, report it.
			...
			PrintFinalStats();
			exit(Options.ErrorExitCode); // not exit() to disable lsan further on.
		}
	}
	...
	void Fuzzer::MutateAndTestOne()
	{
		...
		// 将样本内容拷贝到CurrentUnitData Size
		assert(CurrentUnitData);
		size_t Size = U.size();
		assert(Size <= MaxInputLen && "Oversized Unit");
		memcpy(CurrentUnitData, U.data(), Size);

		// 选择变异的最大长度CurrentMaxMutationLen
		assert(MaxMutationLen > 0);
		size_t CurrentMaxMutationLen = Min(MaxMutationLen, Max(U.size(), TmpMaxMutationLen));
		assert(CurrentMaxMutationLen > 0);

		// 递增变异深度
		for (int i = 0; i < Options.MutateDepth; i++)
		{
			// 总运行次数如果达到了最大运行次数，则退出
			...
			// 对CurrentUnitData Size进行变异
			size_t NewSize = 0;
			if (II.HasFocusFunction &&
				!II.DataFlowTraceForFocusFunction.empty() &&
				Size <= CurrentMaxMutationLen)// 对Mask中指定的需要变异的位置进行变异
			{
				NewSize = MD.MutateWithMask(CurrentUnitData, Size, Size, II.DataFlowTraceForFocusFunction);
			}
			if (!NewSize)// 如果MutateWithMask调用失败或者没有调用，调用Mutate进行变异
			{
			...
			// 如果有新的代码覆盖，则报告新的覆盖，并退出变异
			if (NewCov)
			{
				ReportNewCoverage(&II, { CurrentUnitData, CurrentUnitData + Size });
				break;//我们将在下一轮中编译这个输入
			}

			// 如果设置了ReduceDepth且没有找到新的feature，则退出变异
